...
saveButton.onclick = event => {
  ...
  request.put(storage.session.url('sync'), form).then(
    response => {
      flash(response.data), (storage.editor.item.name = storage.editor.item.data.name);
...
const loadItemsData = e => {
  ...
  request.get(storage.session.url('items.json'), { params: { type: e.detail } }).then(response => {
