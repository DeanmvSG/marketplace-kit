...
saveButton.onclick = (event) => {
  ...
  request.put(
    storage.session.url('sync'),
    form
  ).then(
    (response) => {
      flash(response.data),
      storage.editor.item.name = storage.editor.item.data.name;
...
const reloadItemTypesData = () => {
  request
    .get(storage.session.url('discovery.json'))
    .then(
      response => {
        storage.itemTypes = []
...
const loadItemsData = (e) => {
  ...
  request
    .get(storage.session.url('items.json'), { params: { type: e.detail } })
    .then(response => {
      storage.items.load(response.data.items.results);
...
const renderItemTypeList = () => {
  ...
  for (i in storage.itemTypes) {
    ...
    option.onchange = (e) => {
      if (e.target.checked)
        window.dispatchEvent(new CustomEvent('item-type-selected', { detail: type.name }));
      else
        window.dispatchEvent(new CustomEvent('item-type-deselected', { detail: type.name }));
    }
...
const renderItemList = ({detail}) => {
  ...
  for (i in storage.items.byType(detail)) {
    let page = storage.items.byType(detail)[i];
    if (storage.menu.search.matches(page)){
      let selected = page.name == storage.editor.item.name;
      list.appendChild(renderMenuItem(page, selected));
    }
  }
};
